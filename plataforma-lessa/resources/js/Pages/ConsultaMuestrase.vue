      ...
      <div v-if="muestrasExtra.length">
        ...
        <div class="table-container">
          <table class="table" style="text-align: center; margin-top: 20px;">
            ...
            <tbody>
              <tr v-for="muestra in muestrasExtra" :key="muestra.registro_muestra">
...
export default {
  ...
  methods: {
    async consultarMuestrase() {
      ...
      this.alertMessage = "";
      try {
        const { data: muestrasData } = await axios.get("/api/consulta-muestrase", {
          params: { email: this.$props.user.email, folio: this.folio_muestreoe },
        });
        this.muestrasExtra = muestrasData.data || [];

        const { data: folioData } = await axios.get("/api/folioe-info", {
          params: { email: this.$props.user.email, folio: this.folio_muestreoe },
        });
        this.folioeInfo = folioData.data || {};

        if (!this.muestrasExtra.length && !this.folioeInfo) {
          this.alertMessage = "Folio no encontrado.";
        }
      } catch (error) {
        console.error(error);
        this.alertMessage = "Error al consultar los datos.";
      } finally {
        this.loading = false;
      }
    },
    refresh() {
      this.folio_muestreoe = "";
      this.muestrasExtra = [];
      this.folioeInfo = null;
      this.alertMessage = "";
    },
  },
  props: {
